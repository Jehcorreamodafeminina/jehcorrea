...
export async function sendEmail({ to, toName, subject, htmlContent, bcc }) {
  ...
  const res = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      sender: { name: 'Jeh Corrêa', email: 'contato@jehcorrea.com.br' },
      to: [{ email: to, name: toName || to }],
      bcc: bcc === false ? undefined : [{ email: 'contato@jehcorrea.com.br', name: 'Jeh Corrêa' }],
      subject,
      htmlContent,
    }),
  });
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#FEFCF9;border:1px solid #E0CEBB;max-width:560px;width:100%;">
        <tr>
          <td style="background:#3D2B1A;padding:32px 40px;text-align:center;">
            <p style="margin:0;font-family:Georgia,serif;font-size:22px;font-weight:300;letter-spacing:8px;color:#C4A47A;text-transform:uppercase;">Jeh Corrêa</p>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="background:#F5F0EA;padding:24px 40px;text-align:center;border-top:1px solid #E0CEBB;">
            <p style="margin:0;font-size:11px;color:#9C8070;letter-spacing:1px;">Com carinho, equipe Jeh Corrêa ✦</p>
            <p style="margin:4px 0 0;font-size:11px;color:#C4A47A;letter-spacing:1px;">SEU-DOMINIO-AQUI.com.br</p>
          </td>
        </tr>
      </table>
    </td></tr>
